...
import { getServiceByCategory } from '../services/Services'; // <-- fixed import
...
function DisplayGardening() {
    ...
    const fetchServiceDetails = async () => {
        try {
            setIsLoading(true);
            const response = await getServiceByCategory(category); // <-- fixed usage
            const data = Array.isArray(response.data) ? response.data : (response.data?.services || []);
            setServices(data);
        } catch (error) {
            console.error('Error fetching services:', error);
            toast.error('Failed to load Home Spa services', { theme: 'colored' });
        } finally {
            setIsLoading(false);
        }
    };
            ...
            {isLoading ? (
                ...
                <Row className="g-4 justify-content-center">
                    {services.length > 0 ? services.map((service, idx) => (
                        ...
                        <div className="text-center text-muted">No Home Spa services found.</div>
                    )}
                </Row>
            )}
        </Container>
